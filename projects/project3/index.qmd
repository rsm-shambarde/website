...
First, I loaded the conjoint data directly from the provided Google Drive URL into a Pandas DataFrame. I then proceeded to create the dummy variables:
* For the `brand` attribute, I created `Netflix` (binary 1 if brand is Netflix, 0 otherwise) and `Prime` (binary 1 if brand is Amazon Prime, 0 otherwise) dummy variables. By doing so, **Hulu** naturally became the reference brand, with its effect absorbed into the intercept (or other brand coefficients, as relative utilities are estimated).
* For the `ad` attribute, I created an `Ads` dummy variable (binary 1 if ads are present, 0 otherwise). This makes **ad-free ('No')** the reference category.
* The `price` attribute was already in a numerical format and was directly used.
...
print("numerically stable MNL log-likelihood function defined.")


# --- MLE Estimation Process ---
...
optimization_result_mle = minimize(my_mnl_log_likelihood, initial_betas_mle, 
                                  args=(df_prep,), method='BFGS',
                                  options={'gtol': 1e-4, 'maxiter': 1000}) # <-- ADDED OPTIONS
# --- Presenting MLE Results ---

print("\n--- MLE Optimization Summary ---")
print(f"Optimization successful: {optimization_result_mle.success}")
print(f"Number of iterations: {optimization_result_mle.nit}")
print(f"Number of function evaluations: {optimization_result_mle.nfev}")
print(f"Final negative log-likelihood value: {optimization_result_mle.fun:.4f}")
...
# For comparison with the true values used in data simulation
...
print(true_values_df.to_string(index=False, float_format="%.4f"))
```
...
1.  **Log-Likelihood Function:** This function quantifies how well a given set of parameters explains the observed choices. I used a numerically stable version that returns the positive log-likelihood value, crucial for MCMC algorithms, by employing the log-sum-exp trick for robust calculations.
2.  **Log-Prior Function:** As guided by the assignment, I specified **Normal (Gaussian) prior distributions** for each parameter. Specifically, I used a Normal distribution with a mean of 0 and a standard deviation of 5 for the `Netflix`, `Prime`, and `Ads` coefficients, and a Normal distribution with a mean of 0 and a standard deviation of 1 for the `Price` coefficient. These priors reflect my initial beliefs about the parameters' plausible ranges.
3.  **Log-Posterior Function:** This function combines the log-likelihood and log-prior (by summing them) to represent the logarithm of the posterior probability of the parameters given the data and priors. Robustness checks were included to handle non-finite values.
...
print("Section 5: Imports loaded and data prepared.")


# --- Section 5: Function Definitions (Self-Contained) ---
...
print("Section 5: All Bayesian functions defined.")

# --- Section 5: Metropolis-Hastings MCMC Sampler ---
...
num_mcmc_steps = 11000
mcmc_burn_in = 1000  # Number of initial samples to discard
...
for step_i in range(num_mcmc_steps):
...
# Discard burn-in samples and convert to DataFrame
...
bayesian_summary = posterior_samples_df.describe(percentiles=[0.025, 0.975]).loc[
    ["mean", "std", "2.5%", "97.5%"]
]
print(bayesian_summary.to_string(float_format="%.4f"))


# --- Section 5: Comparison with Maximum Likelihood Approach ---
...
The **trace plot** (which you saw for 'Beta_Price') visually confirms that my MCMC chain explored the parameter space effectively. The values for Beta_Price fluctuated randomly around a stable mean, indicating good mixing and strong convergence of the sampler. The **posterior distribution histogram** for 'Beta_Price' further illustrated this, showing a clear bell-shaped distribution centered very close to the true value of -0.1, which confirms the algorithm successfully sampled the true underlying utility for price.

Looking at the **Bayesian Posterior Summary Statistics**, the posterior means for my parameters are: Netflix (0.9385), Prime (0.5037), Ads (-0.7304), and Price (-0.0995). These values are remarkably close to the true parameters used in the simulation (Netflix: 1.0, Prime: 0.5, Ads: -0.8, Price: -0.1), indicating accurate recovery of the underlying preferences. The standard deviations (e.g., Netflix: 0.1040, Price: 0.0064) show the precision of these estimates, and the 95% credible intervals (e.g., Price: [-0.1123, -0.0862]) provide a clear range of plausible values for each parameter.

Comparing these Bayesian MCMC results with my Maximum Likelihood Estimates (MLE), a strong consistency is evident. The Bayesian posterior means are almost identical to the MLE point estimates, and their respective 95% credible and confidence intervals largely overlap. This close alignment is expected in models with sufficient data where the likelihood dominates the prior. Both estimation methodologies consistently reveal that consumers prefer Netflix over Hulu (the reference brand), show a positive preference for Amazon Prime over Hulu, exhibit a clear disutility when advertisements are present, and strongly prefer lower prices. This convergence across two distinct estimation paradigms provides robust insights into consumer preferences for streaming service attributes.

## 6. Discussion

Having completed both Maximum Likelihood Estimation and Bayesian MCMC analysis of the conjoint data, several key observations can be made about the parameter estimates, assuming this were real-world data rather than simulated. First, consistently across both estimation methods, the parameter estimates provided clear insights into consumer preferences. For instance, my results showed β 
Netflix to be approximately 0.94 and β Prime to be around 0.50 (when compared to Hulu as the reference brand). This means that, all else being equal, consumers derive significantly more utility (or have a stronger preference) for Netflix than for Amazon Prime, and both are preferred over Hulu. This quantitative ranking of brand preferences is crucial for strategic branding and competitive positioning. Furthermore, the estimated β 
price was consistently negative, approximately -0.099. This absolutely makes sense; a negative coefficient for price indicates that as the price of a streaming service increases, the utility consumers receive from it decreases. This aligns perfectly with economic theory and common consumer behavior: people generally prefer lower prices. The magnitude implies that a one-unit increase in price reduces utility by about 0.099 units.

If we were to move beyond this simulated data to analyze "real world" conjoint data, a significant conceptual shift would be required, specifically towards a multi-level (also known as random-parameter or hierarchical) model. In real-world scenarios, consumer preferences are rarely fixed across the entire population; instead, they vary from person to person. To simulate such data, instead of drawing all choices from a single population-level β vector, we would first draw individual-specific βi vectors from a population distribution (e.g., a multivariate normal distribution characterized by a population mean and a covariance matrix). Then, each individual's choices would be generated using their unique βi
​. To estimate the parameters of such a model, our analytical approach would need to transition from estimating a single fixed β to estimating the parameters of the distribution of $\beta_i$s (i.e., the population mean preference and the variance/covariance of preferences across consumers). This often involves more advanced estimation techniques, such as Maximum Simulated Likelihood in a frequentist framework or, more commonly, Hierarchical Bayesian MCMC models, which naturally estimate these individual-level random effects and their population-level distributions.
